...
class TradingController extends Controller
{
    public function manualBuy(Request $request): JsonResponse
    {
        ...
        if (! $user->deriv_account_id) {
            return response()->json(['error' => 'No Deriv account linked.'], 422);
        }

        if ($user->openTrades()->count() >= (int)($params->max_concurrent_trades ?? 2)) {
            return response()->json(['error' => 'Max concurrent trades reached.'], 422);
        }
